/*
  SAZ1051 - OS05L10 (OmniVision 5MP) sensor CMOS / MPP integration.
  Modeled on the Shenshu os04d10 (OmniVision) template; hardware params and
  init sequence extracted from factory libswhisilicon_adapter.so.
 */
...
#include "os05l10_cmos.h"
#include "os05l10_cmos_param.h"
#include "os05l10_sensor_ctrl.h"

/****************************************************************************
 * global variables                                                         *
 ****************************************************************************/
static cis_info g_os05l10_info[OT_ISP_MAX_PIPE_NUM] = {
    [0 ...(OT_ISP_MAX_PIPE_NUM - 1)] {
        ...
        .fast_ae_en = TD_FALSE,
        /* SAZ1051: 板载 OS05L10 挂在 i2c-0。原值 -1 会令 cis_i2c_init()
           open("/dev/i2c-255") 失败，导致整条初始化表与 AE 写全部落空。 */
        .bus_info = { .i2c_dev = 0 },
        .sns_state = TD_NULL,
        ...
        .mode_tbl = {
            {
                OS05L10_VMAX_LINEAR,
                OS05L10_FULL_LINES_MAX_LINEAR,
                OS05L10_FPS_MAX_LINEAR,
                OS05L10_FPS_MIN_LINEAR,
                OS05L10_WIDTH_LINEAR,
                OS05L10_HEIGHT_LINEAR,
                OS05L10_MODE_LINEAR,
                OT_WDR_MODE_NONE,
                "OS05L10_5M_30FPS_10BIT_LINEAR_MODE"
             },
        },
        .i2c.fd = -1,
    }
...
#define OS05L10_REG_VAL_TAB_ROW    63

static td_u32 g_os05l10_ad_gain_table[OS05L10_REG_VAL_TAB_ROW + 1] = {
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    ae_sns_dft->full_lines_max = OS05L10_FULL_LINES_MAX_LINEAR;
    ae_sns_dft->hmax_times = (1000000000) / (OS05L10_VMAX_LINEAR * STANDARD_FPS); /* 1000000000ns, 30fps */
    ...
    if (cis->lines_per500ms == 0) {
        ae_sns_dft->lines_per500ms = (td_u32)(OS05L10_VMAX_LINEAR * max_fps / 2); /*  div 2 */
    } else {
...
static td_void cmos_get_ae_linear_default(cis_info *cis, ot_isp_ae_sensor_default *ae_sns_dft,
    const ot_isp_sns_state *sns_state)
{
    ae_sns_dft->max_again = OS05L10_AGAIN_MAX;
    ae_sns_dft->min_again = OS05L10_AGAIN_MIN;
    ae_sns_dft->max_again_target = ae_sns_dft->max_again;
    ae_sns_dft->min_again_target = ae_sns_dft->min_again;

    ae_sns_dft->max_dgain = OS05L10_DGAIN_MAX;
    ae_sns_dft->min_dgain = OS05L10_DGAIN_MIN;
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    if ((fps > max_fps) || (fps < min_fps)) {
        isp_err_trace("ISP sensor os05l10 Not support Fps: %f\n", fps);
        return;
    }
    ...
    v_blank = vmax - OS05L10_VMAX_VAL_LINEAR;
...
static td_void cmos_slow_framerate_set(ot_vi_pipe vi_pipe, td_u32 full_lines, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    vmax = clip3(vmax, OS05L10_VMAX_LINEAR, OS05L10_FRAME_RATE_MIN);
    sns_state->fl[0] = vmax;
...
static td_void cmos_inttime_update_linear(ot_isp_sns_state *sns_state, td_u32 int_time)
{
    ...
    val = (int_time > OS05L10_FULL_LINES_MAX_LINEAR) ? OS05L10_FULL_LINES_MAX_LINEAR : int_time;
...
static td_void cmos_again_calc_table(ot_vi_pipe vi_pipe, td_u32 *pu32_again_lin, td_u32 *pu32_again_db)
{
    ...
    if (again >= g_os05l10_ad_gain_table[OS05L10_REG_VAL_TAB_ROW]) {
        *pu32_again_lin = g_os05l10_ad_gain_table[OS05L10_REG_VAL_TAB_ROW];
        *pu32_again_db = OS05L10_REG_VAL_TAB_ROW;
    } else {
        for (i = 1; i < OS05L10_REG_VAL_TAB_ROW + 1; i++) {
            if (again < g_os05l10_ad_gain_table[i]) {
                *pu32_again_lin = g_os05l10_ad_gain_table[i - 1];
                *pu32_again_db = i - 1;
                break;
            }
        }
...
static td_void cmos_gains_update(ot_vi_pipe vi_pipe, td_u32 pu32_again, td_u32 pu32_dgain)
{
    ...
    sns_state->regs_info[1].i2c_data[VALUE_IDX].data = 0x00;
    again = (g_os05l10_ad_gain_table[pu32_again] / 0x40);
    sns_state->regs_info[0].i2c_data[PAGE_IDX].data = 0x01;
    sns_state->regs_info[0].i2c_data[AGAIN_IDX].data = (again & 0xFF);
    sns_state->regs_info[0].i2c_data[VALUE_IDX].data = 0x01;
...
static td_s32 cmos_awb_get_default(ot_vi_pipe vi_pipe, ot_isp_awb_sensor_default *awb_sns_dft)
{
    ...
    awb_sns_dft->wb_ref_temp = CALIBRATE_STATIC_TEMP;

    awb_sns_dft->gain_offset[0] = CALIBRATE_STATIC_WB_R_GAIN;
    awb_sns_dft->gain_offset[1] = CALIBRATE_STATIC_WB_GR_GAIN;
    awb_sns_dft->gain_offset[2] = CALIBRATE_STATIC_WB_GB_GAIN;
    awb_sns_dft->gain_offset[3] = CALIBRATE_STATIC_WB_B_GAIN;
...
static td_void cmos_isp_get_dng_default(const ot_isp_sns_state *sns_state, ot_isp_cmos_default *isp_def)
{
    ...
    switch (sns_state->img_mode) {
        case OS05L10_5M_30FPS_10BIT_LINEAR_MODE:
    ...
    isp_def->sns_mode.dng_raw_format.cfa_repeat_pattern_dim.repeat_pattern_dim_row = 2;
    isp_def->sns_mode.dng_raw_format.cfa_repeat_pattern_dim.repeat_pattern_dim_col = 2;
    isp_def->sns_mode.dng_raw_format.black_level_repeat_dim.repeat_row = 2;
    isp_def->sns_mode.dng_raw_format.black_level_repeat_dim.repeat_col = 2;
    ...
    isp_def->sns_mode.dng_raw_format.cfa_plane_color[2] = 2;
    isp_def->sns_mode.dng_raw_format.cfa_pattern[0] = 0;
    isp_def->sns_mode.dng_raw_format.cfa_pattern[1] = 1;
    isp_def->sns_mode.dng_raw_format.cfa_pattern[2] = 1;
    isp_def->sns_mode.dng_raw_format.cfa_pattern[3] = 2;
...
static td_s32 cmos_isp_get_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id = OS05L10_ID;
...
static td_s32 cmos_isp_get_black_level(ot_vi_pipe vi_pipe, ot_isp_cmos_black_level *black_level)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_NONE) {
        ...
        for (i = 0; i < OT_ISP_WDR_MAX_FRAME_NUM; i++) {
            ...
            black_level->auto_attr.black_level[i][2] = BLACK_LEVEL_DEFAULT;
            black_level->auto_attr.black_level[i][3] = BLACK_LEVEL_DEFAULT;
        }
    }
...
static td_void cmos_comm_sns_reg_info_init(cis_info *cis, ot_isp_sns_state *sns_state)
{
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        sns_state->regs_info[0].i2c_data[i].dev_addr = OS05L10_I2C_ADDR;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = OS05L10_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = OS05L10_DATA_BYTE;
    }

    /* Linear Mode Regs */
    sns_state->regs_info[0].i2c_data[PAGE_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[PAGE_IDX].reg_addr = OS05L10_PAGE_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].reg_addr = OS05L10_EXP_HIGH_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].reg_addr = OS05L10_EXP_LOW_ADDR;
    sns_state->regs_info[0].i2c_data[AGAIN_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_IDX].reg_addr = OS05L10_AGAIN_ADDR;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].reg_addr = OS05L10_VMAX_ADDR_H;
    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].reg_addr = OS05L10_VMAX_ADDR_L;
    sns_state->regs_info[0].i2c_data[VALUE_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[VALUE_IDX].reg_addr = 0x01;  /* group-hold / page-valid (page 1) */
    ...
    ot_isp_sns_state *sns_state)
{
    ...
    switch (sns_image_mode) {
        case OS05L10_5M_30FPS_10BIT_LINEAR_MODE:
            sns_state->fl_std = OS05L10_VMAX_LINEAR;
            break;
        default:
            sns_state->fl_std = OS05L10_VMAX_LINEAR;
            break;
...
static td_s32 cmos_isp_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < OS05L10_MODE_MAX; i++) {
        if (sns_image_mode->fps <= cis->mode_tbl[i].max_fps &&
            ...
            image_mode = (os05l10_res_mode)i;
            break;
        }
    }

    if (i >= OS05L10_MODE_MAX) {
...
static void cmos_isp_init(ot_vi_pipe vi_pipe)
{
    ...
    cis->i2c.addr = OS05L10_I2C_ADDR;
    cis->i2c.addr_byte_num = OS05L10_ADDR_BYTE;
    cis->i2c.data_byte_num = OS05L10_DATA_BYTE;
    ...
    os05l10_linear_5m30_10bit_init(cis);
...
static void cmos_isp_exit(ot_vi_pipe vi_pipe)
{
    ...
    if (ret != TD_SUCCESS) {
        isp_err_trace("os05l10 exit failed!\n");
    }
...
static td_void cmos_isp_global_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state->sync_init = TD_FALSE;
    sns_state->img_mode  = OS05L10_5M_30FPS_10BIT_LINEAR_MODE;
    sns_state->wdr_mode  = OT_WDR_MODE_NONE;
    sns_state->fl_std    = OS05L10_VMAX_LINEAR;
    sns_state->fl[0]     = OS05L10_VMAX_LINEAR;
    sns_state->fl[1]     = OS05L10_VMAX_LINEAR;
...
static td_s32 cmos_unregister_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    if (ret != TD_SUCCESS) {
        isp_err_trace("cis_unregister_callback failed!\n");
        return ret;
    }
...
static void cmos_standby(ot_vi_pipe vi_pipe)
{
    ...
    /* stream off (page 0, 0x0100 = 0x00) */
    ret += cis_write_reg(&cis->i2c, 0x00fd, 0x00);
...
static void cmos_restart(ot_vi_pipe vi_pipe)
{
    ...
    /* re-run the verified init sequence */
    os05l10_linear_5m30_10bit_init(cis);
...
ot_isp_sns_obj g_sns_os05l10_obj = {
...
ot_isp_sns_obj *os05l10_get_obj(td_void)
{
    return &g_sns_os05l10_obj;
}
