...
td_s32 cis_i2c_init(cis_info *cis)
{
    ...
    /* SAZ1051 fix: 当 MPP/majestic 没调用 pfn_set_bus_info 时，
       bus_info->i2c_dev 保持初值 -1，(td_u8)(-1) == 255 ->
       open("/dev/i2c-255") 必然失败 -> cis_i2c_init 返回 TD_FAILURE ->
       cmos_isp_init() 提前 return，209 条 sensor 初始化表【从未执行】，
       且后续 AE/AWB 的每次 cis_write_reg 都因 fd<0 静默丢弃。
       这里对无效总线号兜底为 i2c-0（板载 OS05L10 实际所在总线）。 */
    ...
    if (i2c->fd < 0) {
        printf("[sns] open %s FAILED (errno=%d)\n", dev_file, errno);
    ...
    if (ret < 0) {
        printf("[sns] I2C_SLAVE_FORCE 0x%02x FAILED on %s (errno=%d)\n",
               (i2c->addr >> 1), dev_file, errno);
        isp_err_trace("I2C_SLAVE_FORCE error!\n");
        close(i2c->fd);
        i2c->fd = -1;
        return ret;
    }
    printf("[sns] i2c ready: %s fd=%d slave=0x%02x\n", dev_file, i2c->fd, (i2c->addr >> 1));
#endif
